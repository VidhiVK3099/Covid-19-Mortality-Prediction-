...
ui <- fluidPage(
  ...
  navbarPage("Team R",
             ...
             tabPanel("Pearsons Correlation Heatmap",
                      mainPanel(
                        dateInput("date", "Select Date:", value = "2023-01-01"), # Date input for selecting date
...
server <- function(input, output) {
  ...
  output$heatmap_plot <- renderPlot({
    ...
    date_filtered_data <- dw %>% filter(date == as.Date(input$date))
    ...
    print(heatmap_plot)  # Print the plot for debugging
    
    return(heatmap_plot)  # Return the heatmap plot
  })
